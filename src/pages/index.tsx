...
import IPLogo from '../../public/images/icons/favicon.png';
...
const Login: NextPage = () => {
    ...
    const { t, i18n } = useTranslation(['login', 'common', 'messages']);
    ...
    const loginMutation = useMutation({
        mutationFn: async (values: LoginValue) => {
    ...
    const requestResetPasswordMutation = useMutation({
        mutationFn: async (values: { email: string }) => {
            ...
            const res = await fetch(`/api/staff/auth/resetPassword`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'accept-language': currentLocale,
        ...
        onError: (error: Error) => {
            forgotPasswordToast.update('error', t(error.message));
        },
        onSuccess: () => {
    ...
    const onLoginHandler = () => {
        loginForm.validateFields().then((values: LoginValue) => {
    ...
    return (
        <div className="flex items-center justify-center min-h-screen loginBackground">
            ...
            <div className="max-w-[500px] mx-4 w-full border-solid border-2 border-gray-200 rounded bg-white px-7 py-10">
                ...
                <div>
                    <Form layout="vertical" form={loginForm} name="Login Form" onKeyUp={handleKeyUp} tabIndex={0}>
                        <Form.Item
                            ...
                            rules={[
                                { required: true },
                                { pattern: usernameRegex, message: t('messages:error.invalid', { label: '${label}' }) as string },
                                { min: 4, message: t('messages:error.string too short', { label: '${label}', min: '4' }) as string },
                                { max: 20, message: t('messages:error.string too long', { label: '${label}', max: '20' }) as string },
                            ]}
                        >
                        ...
                        <Form.Item
                            ...
                            rules={[
                                { required: true },
                                { min: 6, message: t('messages:error.string too short', { label: '${label}', min: '6' }) as string },
                                { max: 20, message: t('messages:error.string too long', { label: '${label}', max: '20' }) as string },
                            ]}
                        >
...
export const getServerSideProps: GetServerSideProps = async ({ req, locale }) => {
    ...
    return {
        props: {
            ...(await serverSideTranslations(locale as string, ['login', 'common', 'messages'])),
        },
    };
};
